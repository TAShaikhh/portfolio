  ...
  return (
    <Section id="experience" title="Experience">
      <div className="relative">
        {/* Timeline line with flowing animation */}
        <div className="absolute left-3 md:left-1/2 transform md:-translate-x-1/2 top-0 h-[calc(100%-2rem)] w-0.5 z-0 hidden sm:block overflow-hidden">
          {/* Base line */}
          <div className="absolute inset-0 bg-[#00A6ED]/20"></div>
          {/* Animated gradient overlay */}
          <div className="absolute inset-0 bg-gradient-to-b from-transparent via-[#00A6ED] to-transparent opacity-50 animate-flow-down"></div>
          {/* Glow effect */}
          <div className="absolute inset-0 shadow-[0_0_10px_#00A6ED]"></div>
            ...
            return (
              <AnimatedCard key={index} delay={0.2 + index * 0.1}>
                <div className={`sm:flex ${isEven ? 'sm:flex-row' : 'sm:flex-row-reverse'} items-center gap-4 md:gap-8`}>
                  {/* Timeline dot */}
                  <div className="absolute left-3 md:left-1/2 transform md:-translate-x-1/2 w-6 h-6 rounded-full bg-secondary border-2 border-[#00A6ED] z-10 hidden sm:block shadow-[0_0_10px_#00A6ED]"></div>

                  <div className={`sm:w-1/2 ${isEven ? 'sm:pr-10 md:pr-16' : 'sm:pl-10 md:pl-16'}`}>
                    <Card 
                      className={`bg-secondary/20 border-0 transition-all duration-300 cursor-pointer h-full
                        ${isExpanded ? 'shadow-[0_0_25px_rgba(0,166,237,0.3)]' : 'shadow-[0_0_15px_rgba(0,166,237,0.1)]'}
                        hover:shadow-[0_0_20px_rgba(0,166,237,0.2)] hover:bg-secondary/30 ${colorClasses.hover} group`}
